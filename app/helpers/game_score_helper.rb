module GameScoreHelper
  def top_3_scorers_message(game_name)
    ...
    if top_3.length == 1
      ...
      return sanitize "Top scorers are : <br/> N°1 #{top_3.first.user.username} score -> #{top_3.first.score} <br/>
                       N°2 #{top_3.second.user.username} score -> #{top_3.second.score}"
    else
      sanitize "Top scorers are : <br/> N°1 #{top_3.first.user.username} score -> #{top_3.first.score} <br/>
